...
<!-- 		
			...
			session에서 memberId 값이 있는지 없는지 체크하기 위해서는 if 문이 필요
	 		...
	 		jstl의 경우 jar 파일만 있다고 쓸 수 있는 것이 아니라 taglib을 작성해 주어야 함 
-->
...
<!-- 		
			jstl에는 다양한 종류의 태그가 있는데 그 중 제어문을 담당하는 core태그를 사용하는 것
	 		...
	 		특이하게 test다음에 EL인 $를 써서 session에 저장하는 값을 불러오는데
	 		sessionScope는 session에 접근하기 위해 작성해 주는 내장 객체이며 생략 가능
	 		하지만 request에도 member Id가 있다면 둘을 구별하기 위해서 꼭 써줘야 함 -> 키값이 중복되지 않는다면 생략해도 됨
	 		memberId == null이면 로그인 폼을 보여주고 memberId != null이면(Not Equal) 로그인했다는 것
