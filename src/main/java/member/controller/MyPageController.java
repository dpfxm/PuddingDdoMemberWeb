...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		 ...
		 * myPage를 호출하는 URL은 --/member/myPage.do?memberId=admin과 같은 형태로
		 * 쿼리스트링 필요! 쿼리스트링으로 보낸 데이터를 받기 위해서는 request의
		 * getParameter() 메소드에 키값 memberId로 일치시키기
		 */
		try {
			String memberId = request.getParameter("memberId");
			// Service에 로그인한 사용자의 아이디를 넘겨주고 실행결과로 member 객체 받음
			MemberService mService = new MemberService();
			Member member = mService.selectOneById(memberId);
			if (member != null) {
				request.setAttribute("member", member);
				// 변수 안쓰고 이동할 페이지 지정 및 이동(forward() 호출)
			 ...
			 * 오류 메시지를 오류 페이지를 통해서 볼 수 있도록 request에
