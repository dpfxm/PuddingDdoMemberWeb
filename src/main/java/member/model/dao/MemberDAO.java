...
public class MemberDAO {
	...
	public Member selectLoginCheck(Connection conn, Member member) throws SQLException {
		 ...
		 * 예외처리 방법은 2가지로
		 * 첫 번째, throw를 호출하여 그곳에서 처리하도록 하는 방법
		 * 두 번째, try ~ catch를 사용해서 그 자리에서 처리하는 방법
		 * 여기서는 throw의 SQLException 사용
		 */
		PreparedStatement pstmt = conn.prepareStatement(query);
		/*
		 * 위치홀더가 2개이고 여기에 값을 세팅해 주어야 입력한 값이
		 ...
		 * rSet은 그대로 못 쓰고 rSet을 member로 바꿔주는 작업이 필요한데
		 ...
		 * 1개만 가져오면 if, 여러 개를 가져올 경우에는 while 문 사용
	...
	public Member selectOneById(Connection conn, String memberId) throws SQLException {
		 ...
		 * 메소드로 rSet을 member 객체로 변환해 줌
		 * 변환 이유 : Oracle 시스템, Java 시스템처럼 
		 * 다른 시스템들이 만나서 데이터를 주고받기 때문
		 * 자원해재 한 후, member 리턴
		 */
	...
	public Member rSetToMember(ResultSet rSet) throws SQLException {
		/*
		 * rSet에 있는 값을 담을 member 객체를 선언해 주고 rSet.getStiring()등의 메소드를 이용하여
		 * member에 값을 넣어줌. 값을 넣을 땐 setter 메소드를 이용하여 member에 값을 넣어줌
		 * rSEt.getString() 메소드는 전달값으로 컬러명을 적어줘야하는데 이때 오타 조심
