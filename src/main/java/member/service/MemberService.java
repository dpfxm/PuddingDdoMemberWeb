...
public class MemberService {
	...
	public MemberService() {
		...
		mDao = new MemberDAO();
		/*
		 * conn은 멤버변수라고 하는데 그 초기화는 생성자에서 해줌
		 * getConnection 메소드는 static 메소드라
		 * 객체 생성 없이도 호출이 가능하여 다음과 같이 사용
		 * 싱글톤이 적용되어 conn은 한 번만 생성됨
	...
	/*
	 * selectLoginCheck 메소드는 member 객체를 전달 받아서
	 * DAO에 전달해주는 역할
	 * 서비스에서는 DBMS 연결을 만들어서 DAO에 전달
	 * 마지막으로 쿼리 성공여부에 따라 commit/rollback을 함(DML 경우에만)
	...
	public Member selectLoginCheck(Member member) {
		Member mOne = null;
		/*
		 * conn은 이미 전에 선언해서 연결을 만들어 놨기 때문에 메소드만 DAO에 자동생성
